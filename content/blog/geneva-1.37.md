...
In this tutorial, you'll learn how to build a high-throughput feature engineering pipeline on top of [LanceDB's Feature Engineering project: Geneva](/docs/geneva/). 

We'll work with the [Oxford-IIIT Pets dataset (cats and dogs)](https://www.robots.ox.ac.uk/~vgg/data/pets/). We'll start with a single-column table of images. Then we'll expand it into something far richer. We'll add per-image file sizes, dimensions, captions generated by a GPU model, and dense vector embeddings suitable for similarity search.

...
weight: 3
aliases: ["/docs/concepts/indexing/scalar-index/", "/docs/concepts/indexing/scalar-index"]
---
...
table = db.open_table("my_vectors")

# Create BTREE index (default)
table.create_scalar_index("book_id")
...
await tbl.createIndex("book_id");
await tbl.createIndex("publisher", { config: lancedb.Index.bitmap() })
{{< /code >}}
...
# Vector search with scalar prefilter
result = table.search([1.2] * 1536) \
    .where("book_id != 3") \
    .limit(10) \
    .to_pandas()
{{< /code >}}
...
{{< admonition note "FixedSizeBinary Requirements" >}}

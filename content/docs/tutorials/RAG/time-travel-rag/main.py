...
def run_workflow():
    ...
    print(f"✅ Table '{config.TABLE_NAME}' created. Version: {tbl.version}, Rows: {len(tbl)}")
    ...
    if update_docs_1:
        ...
        print(f"✅ Data added to '{tbl.name}'. New Version: {tbl.version}, Total Rows: {len(tbl)}")
    ...
    for version_num in range(1, tbl.version + 1):
        ...
        print(f"✅ Successfully checked out Version {version_num} of '{config.TABLE_NAME}'. Total rows: {len(tbl_to_search)}")
    ...
    print(f"✅ Table '{config.EXPERIMENTAL_TABLE_NAME}' created. Version: {exp_tbl.version}, Rows: {len(exp_tbl)}")
